...
import LoVe.Lectures.LoVe08_Metaprogramming_Demo
...
theorem ab (a b : Prop) (h : a ∨ b) :
  a ∨ b :=
  by destro_and

theorem abcdef_1 (a b c d e f : Prop) (h : a ∧ b ∧ c ∧ d ∧ e ∧ f) :
  f ∧ e ∧ f ∧ a ∧ d ∧ b ∧ c ∧ a :=
  by destro_and

theorem abcdef_2 (a b c d e f : Prop) (h : a ∧ ((b ∧ ((c ∧ d) ∧ e)) ∧ f)) :
  f ∧ e ∧ f ∧ a ∧ d ∧ b ∧ c ∧ a :=
  by destro_and

theorem abcdef_3 (a b c d e f : Prop) (h : a ∧ (b ∧ (c ∧ (d ∧ e ∧ f)))) :
  f ∧ (e ∧ (f ∧ (a ∧ d ∧ b ∧ c ∧ a))) :=
  by destro_and
...
def constInExpr (name : Name) (e : Expr) : Bool :=
  match e with
  | Expr.const name' _ => name' == name
  | Expr.app e₁ e₂ => constInExpr name e₁ || constInExpr name e₂
  | Expr.lam _ ty body _ => constInExpr name ty || constInExpr name body
  | Expr.forallE _ ty body _ => constInExpr name ty || constInExpr name body
  | Expr.letE name ty val body _ =>
    constInExpr name ty || constInExpr name val || constInExpr name body
...
def constsInExpr (names : List Name) (e : Expr) : Bool :=
  List.and (List.map (fun name ↦ constInExpr name e) names)
  ...
  do
    ...
    for (name, info) in SMap.toList (Environment.constants env) do
      if isTheorem info && ! ConstantInfo.isUnsafe info
         && constsInExpr names (ConstantInfo.type info) then
        logInfo m!"{name}"
